...
namespace SalePoint.API.Controllers
{
    ...
    public class ProductController(IProductRepository productRepository) : Controller
    {
        ...
        //[HttpGet("All/pageNumber/{pageNumber}/pageSize/{pageSize}/keyWord/{keyWord}")]
        [HttpPost("All")]
        public async Task<ActionResult> GetAllProducts([FromForm] ProductQueryParameters productQuery)
        {
            try
            {
                return Json(await _productRepository.GetAllProducts(productQuery.PageNumber, productQuery.PageSize, productQuery.KeyWord));
            }
            catch (Exception ex)
            {
                return BadRequest(new { isError = true, message = ex.Message });
