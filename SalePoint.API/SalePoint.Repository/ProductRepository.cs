    ...
    public class ProductRepository(IConfiguration configuration) : IProductRepository
    {
        ...
        public async Task<int> CreateProduct(Product product)
        {
            try
            {
                ...
                dynamicParameters.Add("RouteImage", product.RouteImage);
        ...
        public async Task<ProductModel> GetAllProducts(int? pageNumber, int? pageSize, string? keyWord)
        {
            try
            {
                ...
                //using var multi = await conn.QueryMultipleAsync("GetAllProducts", param: parameters, commandType: CommandType.StoredProcedure);

                //// Primer resultado: Lista de productos
                //products.Products = await multi.ReadAsync<ResponseProduct>();

                //// Segundo resultado: Llenar el objeto Filters
                //products.Filters = await multi.ReadSingleAsync<Filters>();

                string? productResult = await conn.QueryFirstOrDefaultAsync<string>("GetAllProducts", param: parameters, commandType: CommandType.StoredProcedure);

                if (productResult != null)
                    products = JsonConvert.DeserializeObject<ProductModel>(productResult) ?? new ProductModel();
