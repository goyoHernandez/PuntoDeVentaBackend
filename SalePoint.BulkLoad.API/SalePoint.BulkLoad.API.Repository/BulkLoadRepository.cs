...
namespace SalePoint.BulkLoad.API.Repository
{
    public class BulkLoadRepository : IBulkLoadRepository
    {
        ...
        public async Task<List<BulkUpgradeErrors>> UpgradeBulkLoadProducts(IFormFile formFile, int userId)
        {
            try
            {
                List<UpgradeProductType> productTypes = [];
                ...
                List<BulkUpgradeErrors> bulkUpgradeErrors = (await conn.QueryAsync<BulkUpgradeErrors>("BulkUpgradeProducts", new { products = dataTable.AsTableValuedParameter("[dbo].[UpgradeProductType]") }, commandType: CommandType.StoredProcedure)).ToList();
        ...
        private static DataTable ListToDataTableUpgradeProducts(List<UpgradeProductType> upgradeProductTypes, int userId)
        {
            ...
            foreach (UpgradeProductType upgradeProductType in upgradeProductTypes)
            {
                ...
                row[nameof(upgradeProductType.ProductId)] = upgradeProductType.ProductId < 1 ? DBNull.Value : upgradeProductType.ProductId;
                row[nameof(upgradeProductType.Name)] = upgradeProductType.Name is null ? "" : upgradeProductType.Name;
                row[nameof(upgradeProductType.ExpirationDate)] = string.IsNullOrEmpty(upgradeProductType.ExpirationDate) ? DBNull.Value : upgradeProductType.ExpirationDate;
